...
pub fn sumcheck_verifier<F: Field, EF: ExtField<F>, Transcript>(
    d: usize,
    degree: usize,
    mut sum: EF,
...
mod test {
    use crate::{
        basefold::sumcheck::{sumcheck_prover, sumcheck_verifier},
        field::{goldilocks::Goldilocks, Field},
        hash::rust_crypto::{RustCryptoReader, RustCryptoWriter},
    };
    ...
    fn test_sumcheck_single() {
        ...
        for d in 0..8 {
            ...
            let (red0, mut rs0) = sumcheck_prover(&mut writer, d, sum, &point, &mut poly).unwrap();

            assert_eq!(poly.len(), 1 << (k - d));
            let (z_poly, z_eq) = point.split_at(k - d);
